...
local function trim_spell_name(string_spell_name)
    ...
    local is_greater =
        (string.find(string_spell_name_lower, ', greater') ~= nil)
    local is_lesser = (string.find(string_spell_name_lower, ', lesser') ~= nil)
    local is_communal = (string.find(string_spell_name_lower, ', communal') ~=
                            nil)
    local is_mass = (string.find(string_spell_name_lower, ', mass') ~= nil)
    local is_maximized = (string.find(string_spell_name_lower, 'maximized') ~=
                             nil)
    local is_empowered = (string.find(string_spell_name_lower, 'empowered') ~=
                             nil)
    local is_quickened = (string.find(string_spell_name_lower, 'quickened') ~=
                             nil)
    ...
    local number_name_end = (string_spell_name:find('D', string.len(
                                                        string_spell_name)) or
                                string_spell_name:find('M', string.len(
                                                           string_spell_name)))
    if number_name_end then
        string_spell_name = string_spell_name:sub(1, number_name_end - 1)
    end
...
local function replace_action_nodes(node_spell, node_spellset,
                                    number_spell_level, node_reference_spell,
                                    is_maximized, is_empowered)
    if node_reference_spell then
        if node_reference_spell.getChild('actions') then
            ...
            local node_spell_new = SpellManager.addSpell(node_reference_spell,
                                                         node_spellset,
                                                         number_spell_level)
            ...
            local node_spell_new_damage =
                node_spell_new.getChild('actions').getChild('damage')
            if node_spell_new_damage then
                if is_empowered then
                    DB.setValue(node_spell_new_damage, 'meta', 'string',
                                'empower')
                end
                if is_maximized then
                    DB.setValue(node_spell_new_damage, 'meta', 'string',
...
local function add_spell_description(node_spell, node_reference_spell)
    if node_reference_spell and node_spell then
        if DB.getValue(node_spell, 'description', '') == '' or
            DB.getValue(node_spell, 'description', '') == '<p></p>' then
            DB.deleteNode(node_spell.createChild('description'))
            local string_full_description =
                DB.getValue(node_reference_spell, 'description', '<p></p>')
            DB.setValue(node_spell, 'description_full', 'formattedtext',
                        string_full_description)
            DB.setValue(node_spell, 'description', 'formattedtext',
                        string_full_description)
...
local function add_spell_information(node_spell, node_reference_spell)
    if node_reference_spell and node_spell then
        for _, node_reference_spell_subnode in pairs(
                                                   node_reference_spell.getChildren()) do
            ...
            if string_node_name ~= 'description' and string_node_name ~= 'name' then
                if not node_spell.getChild(string_node_name) then
                    local string_node_type =
                        node_reference_spell_subnode.getType()
                    local node_spell_subnode =
                        node_spell.createChild(string_node_name,
                                               string_node_type)
...
local function replace_spell_actions(nodeSpell)
    local string_spell_name, is_maximized, is_empowered = trim_spell_name(
                                                              DB.getValue(
                                                                  nodeSpell,
                                                                  'name'))
    ...
    local array_modules = {
        ['SpellbookExtended'] = {
            ['name'] = '@PFRPG - Spellbook Extended',
            ['prefix'] = 'spelldesc.'
        },
        ['Spellbook'] = {
            ['name'] = '@PFRPG - Spellbook',
            ['prefix'] = 'spelldesc.'
        }
    }
    ...
    for _, table_module_data in pairs(array_modules) do
        node_reference_spell = DB.findNode(
                                   table_module_data['prefix'] ..
                                       string_spell_name ..
                                       table_module_data['name'])
        if node_reference_spell then break end
    end
    local number_spell_level = tonumber(
                                   nodeSpell.getChild('...').getName():gsub(
                                       'level', '') or 0)
    if number_spell_level and string_spell_name and node_reference_spell then
        local node_spellset = node_spell.getChild('.....')
        local node_new_spell = replace_action_nodes(nodeSpell, node_spellset,
                                                    number_spell_level,
                                                    node_reference_spell,
                                                    is_maximized, is_empowered)
...
local function find_spell_nodes(nodeEntry)
    for _, nodeSpellset in
        pairs(nodeEntry.createChild('spellset').getChildren()) do
        for _, nodeSpellLevel in pairs(nodeSpellset.createChild('levels')
                                           .getChildren()) do
            for _, nodeSpell in pairs(nodeSpellLevel.createChild('spells')
                                          .getChildren()) do
...
local function string_to_table(string_input)
    ...
    repeat
        ...
        table.insert(table_output, string.sub(string_input, number_field_start,
                                              number_nexti - 1))
...
local function add_malady_link(node_malady, node_npc)
    local table_malady_npcs =
        string_to_table(DB.getValue(node_malady, 'npc')) or {}
    if table_malady_npcs ~= {} then
        for _, string_malady_linked_npc in pairs(table_malady_npcs) do
            local string_difficulty_class =
                (string_malady_linked_npc:match(' %(DC %d+%)')) or ''
            string_malady_linked_npc = string_malady_linked_npc:gsub(
                                           ' %(DC %d+%)', '')
            string_malady_linked_npc = string.lower(
                                           string_malady_linked_npc:gsub('%A',
                                                                         ''))
            ...
            if string_malady_linked_npc == string_npc_name then
                ...
                local string_malady_link =
                    ('<linklist><link class="referencedisease" recordname="' ..
                        ...
                        '</link></linklist>')
                DB.setValue(node_npc, 'text', 'formattedtext',
                            string_malady_link .. string_description)
            end
        end
    end
end
    ...
    if DiseaseTracker then
        if DB.getValue(node_npc, 'name') then
            if DB.findNode('reference.diseases@*') then
                for _, node_malady in pairs(
                                          DB.findNode('reference.diseases@*')
                                              .getChildren()) do
                    add_malady_link(node_malady, node_npc)
                end
            end
            if DB.findNode('disease') then
                for _, node_malady in
                    pairs(DB.findNode('disease').getChildren()) do
                    add_malady_link(node_malady, node_npc)
                end
            end
        end
...
local function add_ability_automation(node_npc, string_ability_name,
                                      table_ability_information, number_rank,
                                      string_parenthetical)
    if (not node_npc or string_ability_name == '' or
        not table_ability_information or table_ability_information == {} or
        (table_ability_information['daily_uses'] and
            table_ability_information['daily_uses'] < 0) or
        table_ability_information['level'] < 0 or
        table_ability_information['level'] > 9 or
    ...
    local node_spellclass = node_spellset.createChild(
                                table_ability_information['string_ability_type'] or
                                    'Abilities')
    local node_spelllevel = node_spellclass.createChild('levels').createChild(
                                'level' .. table_ability_information['level'])
    local node_ability = node_spelllevel.createChild('spells').createChild()

    -- set up spellset and intermediate subnodes
    DB.setValue(node_spellclass, 'label', 'string',
                table_ability_information['string_ability_type'])
    DB.setValue(node_spellclass, 'castertype', 'string', 'spontaneous')
    DB.setValue(node_spellclass,
                'availablelevel' .. table_ability_information['level'],
                'number', table_ability_information['daily_uses'] or 1)
    DB.setValue(node_spellclass, 'cl', 'number', 0)
    DB.setValue(node_spelllevel, 'level', 'number',
                table_ability_information['level'])

    -- set name and description
    DB.setValue(node_ability, 'name', 'string', string_ability_name)
    DB.setValue(node_ability, 'description', 'string',
                (table_ability_information['description'] or '') ..
                    (string_parenthetical or ''))
    if table_ability_information['perday'] then
        DB.setValue(node_ability, 'prepared', 'number',
                    table_ability_information['perday'])
    end

    -- create actions
    local node_actions = node_ability.createChild('actions')
    for string_name_action, table_action_information in pairs(
                                                            table_ability_information['actions']) do
        ...
        for string_node_name, table_node_info in pairs(table_action_information) do
            if string_node_name == 'damagelist' or string_node_name ==
                'heallist' then
                for string_damage_name, table_damage_information in pairs(
                                                                        table_node_info) do
                    local node_damage =
                        node_action.createChild(string_node_name).createChild(
                            string_damage_name)
                    for string_damagenode_name, table_damagenode_info in pairs(
                                                                             table_damage_information) do
                        if table_damagenode_info['type'] and
                            table_damagenode_info['value'] then
                            if table_damagenode_info['tiermultiplier'] then
                                if table_damagenode_info['type'] == 'string' then
                                    local string_result = string.format(
                                                              ...
                                                              (table_damagenode_info['tiermultiplier'] *
                                                                  (number_rank or
                                                                      1)))
                                    DB.setValue(node_damage,
                                                string_damagenode_name,
                                                table_damagenode_info['type'],
                                                string_result)
                                ...
                                DB.setValue(node_damage, string_damagenode_name,
                                            table_damagenode_info['type'],
                                            table_damagenode_info['value'])
                            end
                        end
                ...
                if table_node_info['type'] and table_node_info['value'] then
                    if table_node_info['tiermultiplier'] then
                        local result = string.format(table_node_info['value'],
                                                     (table_node_info['tiermultiplier'] *
                                                         (number_rank or 1)))
                        DB.setValue(node_action, string_node_name,
                                    table_node_info['type'], result)
                    else
                        DB.setValue(node_action, string_node_name,
                                    table_node_info['type'],
                                    table_node_info['value'])
                    end
                end
            end
        end
...
local function hasSpecialAbility(nodeActor, sSearchString, bFeat, bTrait,
                                 bSpecialAbility)
    if not nodeActor or not sSearchString or
        (not bFeat and not bTrait and not bSpecialAbility) then return false; end

    local sLowerSpecAbil = string.lower(sSearchString);
    local sSpecialQualities = string.lower(
                                  DB.getValue(nodeActor, '.specialqualities', ''));
    local sSpecAtks =
        string.lower(DB.getValue(nodeActor, '.specialattacks', ''));
    ...
    if bFeat and sFeats:match(sLowerSpecAbil, 1) then
        ...
        local sParenthetical =
            sSpecAtks:match(sLowerSpecAbil .. ' %((.+)%)', 1) or
                sFeats:match(sLowerSpecAbil .. ' %((.+)%)', 1)
        return true, (nRank or 1), sParenthetical
    elseif bSpecialAbility and (sSpecAtks:match(sLowerSpecAbil, 1) or
        sSpecialQualities:match(sLowerSpecAbil, 1)) then
        local nRank = tonumber(sSpecAtks:match(sLowerSpecAbil .. ' (%d+)', 1) or
                                   sSpecialQualities:match(
                                       sLowerSpecAbil .. ' (%d+)', 1))
        local sParenthetical =
            sSpecAtks:match(sLowerSpecAbil .. ' %((.+)%)', 1) or
...
local function parse_breath_weapon(string_parenthetical,
                                   table_ability_information)
    if not string_parenthetical then return; end
    local string_parenthetical_lower = string.lower(
                                           ', ' .. string_parenthetical .. ',')
    local dice_damage, string_damage_type =
        string_parenthetical_lower:match(',%s(%d%d*d*d%d+)%s*(%l+)[.+]?')
    -- Debug.chat(string_parenthetical_lower, dice_damage, string_damage_type)
    local string_save_type, number_save_dc, string_save_half =
        string_parenthetical_lower:match(
    ...
    local dice_recharge = string_parenthetical_lower:match(
                              ',%susable%severy%s(%d%d*d*d%d+)%srounds[.+]?')
    -- Debug.chat(string_parenthetical_lower, dice_recharge)

    table_ability_information['actions']['breathweapondmg']['damagelist']['primarydamage']['dice']['value'] =
        dice_damage
    ...
    if string_save_type and string_save_type ~= '' then
        table_ability_information['actions']['breathweaponsave']['savetype']['value'] =
            string_save_type
    end
    table_ability_information['actions']['breathweaponsave']['savedcmod']['value'] =
        number_save_dc
    table_ability_information['actions']['breathweaponsave']['onmissdamage']['value'] =
        string_save_half
    if dice_recharge and dice_recharge ~= '' then
        if dice_recharge:sub(1, 2) == '1d' then
            dice_recharge = dice_recharge:gsub('1d', 'd')
        end
        table_ability_information['actions']['breathweaponrecharge']['durdice']['value'] =
            dice_recharge
    end
end
...
local function search_for_abilities(node_npc) -- luacheck: no max line length
        ...
        ['Arcane Strike'] = {
            ...
            ['actions'] = {
                ['zeffect-1'] = {
                    ...
                    ['label'] = {
                        ['type'] = 'string',
                        ['value'] = ('Arcane Strike; DMG: 1; DMGTYPE: magic')
                    },
                    ['targeting'] = {['type'] = 'string', ['value'] = 'self'},
                    ['type'] = {['type'] = 'string', ['value'] = 'effect'}
                }
        ...
        ['Combat Expertise'] = {
            ...
            ['actions'] = {
                ['zeffect-1'] = {
                    ...
                    ['label'] = {
                        ['type'] = 'string',
                        ['value'] = 'Combat Expertise; ATK: -1 [-QBAB] ,melee; AC: 1 [QBAB] dodge'
                    },
                    ['targeting'] = {['type'] = 'string', ['value'] = 'self'},
                    ['type'] = {['type'] = 'string', ['value'] = 'effect'}
                }
        ...
        ['Critical Focus'] = {
            ...
            ['actions'] = {
                ['zeffect-1'] = {
                    ...
                    ['label'] = {
                        ['type'] = 'string',
                        ['value'] = 'Critical Focus; CC: +4 circumstance'
                    },
                    ['targeting'] = {['type'] = 'string', ['value'] = 'self'},
                    ['type'] = {['type'] = 'string', ['value'] = 'effect'}
                }
        ...
        ['Deadly Aim'] = {
            ...
            ['actions'] = {
                ['zeffect-1'] = {
                    ...
                    ['label'] = {
                        ['type'] = 'string',
                        ['value'] = 'Deadly Aim; ATK: -1 [-QBAB] ,ranged; DMG: 2 [QBAB] [QBAB] ,ranged'
                    },
                    ['targeting'] = {['type'] = 'string', ['value'] = 'self'},
                    ['type'] = {['type'] = 'string', ['value'] = 'effect'}
                }
        ...
        ['Defended Movement'] = {
            ...
            ['actions'] = {
                ['zeffect-1'] = {
                    ...
                    ['label'] = {
                        ['type'] = 'string',
                        ['value'] = 'Defended Movement; AC: 4 ,,opportunity'
                    },
                    ['targeting'] = {['type'] = 'string', ['value'] = 'self'},
                    ['type'] = {['type'] = 'string', ['value'] = 'effect'}
                }
        ...
        ['Furious Focus'] = {
            ...
            ['actions'] = {
                ['zeffect-1'] = {
                    ...
                    ['label'] = {
                        ['type'] = 'string',
                        ['value'] = 'Furious Focus; IF: CUSTOM(Power Attack 2-H); ATK: 1 [QBAB] ,melee'
                    },
                    ['targeting'] = {['type'] = 'string', ['value'] = 'self'},
                    ['type'] = {['type'] = 'string', ['value'] = 'effect'}
                }
        ...
        ['Mobility'] = {
            ...
            ['actions'] = {
                ['zeffect-1'] = {
                    ['label'] = {
                        ['type'] = 'string',
                        ['value'] = 'Mobility; AC: 4 dodge,opportunity'
                    },
                    ['targeting'] = {['type'] = 'string', ['value'] = 'self'},
                    ['type'] = {['type'] = 'string', ['value'] = 'effect'}
                }
    ...
    for string_ability_name, table_ability_information in pairs(array_abilities) do
        ...
        elseif table_ability_information['string_ability_type'] == 'Traits' then
            is_trait = true
        elseif table_ability_information['string_ability_type'] ==
            'Special Abilities' then
        ...
        if is_match then
            if string_parenthetical and string_ability_name == 'Breath Weapon' then
                parse_breath_weapon(string_parenthetical,
                                    table_ability_information)
            end

            add_ability_automation(node_npc, string_ability_name,
                                   table_ability_information, number_rank,
                                   string_parenthetical)
        end
    end
